...
- (LEEConfigToString)LeeContent{
    ...
    return ^(NSString *str){
        ...
        return  weakSelf.LeeAddContent(^(UILabel *label) {
            ...
            NSDictionary *attributes = @{ NSFontAttributeName:[UIFont systemFontOfSize:14], NSParagraphStyleAttributeName:paragraphStyle,NSForegroundColorAttributeName:LColor(74, 74, 74)};//字体大小 行间距  颜色
            NSAttributedString *butedString =  [[NSAttributedString alloc]initWithString:str attributes:attributes];
            label.attributedText = butedString;
...
- (LEEConfigToStringAndBlock)LeeDestructiveAction{
    ...
    return ^(NSString *title , void(^block)(void)){
        ...
        return weakSelf.LeeAddAction(^(LEEAction *action) {
            ...
            action.title = title;
             //2017-8-3号修改  backgroundColor && titleColor
...
- (LEEConfigToBlockAndBlock)LeeOpenAnimationConfig{
    ...
    return ^(void(^block)(void (^animatinvoidgBlock)(void) , void (^animatedBlock)(void))){
...
- (void)updateAlertLayoutWithViewWidth:(CGFloat)viewWidth ViewHeight:(CGFloat)viewHeight{
    ...
    if (isShowingKeyboard) {
        ...
        if (keyboardFrame.size.height) {
            ...
            CGFloat tempAlertViewY = keyboardY - tempAlertViewHeight + 5;

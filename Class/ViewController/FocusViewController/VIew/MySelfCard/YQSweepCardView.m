...
- (void)commonInit{
    ...
    _contentInsets = UIEdgeInsetsMake(10, 10, 5, 10);
...
//    [self addGestureRecognizer:pan];
    ...
    // 添加向左轻扫手势
    UISwipeGestureRecognizer *swipeGestureRecognizerLeft = [[UISwipeGestureRecognizer alloc] initWithTarget:self action:@selector(swipeAction:)];
    [self addGestureRecognizer:swipeGestureRecognizerLeft];
    swipeGestureRecognizerLeft.direction = UISwipeGestureRecognizerDirectionLeft;
    
    // 添加向右轻扫手势
    UISwipeGestureRecognizer *swipeGestureRecognizerRight = [[UISwipeGestureRecognizer alloc] initWithTarget:self action:@selector(swipeAction:)];
...
- (void)moveOut:(YQSweepCardItem *)movingItem completion:(void(^)(void))completion{
    ...
    [UIView animateWithDuration:_animationDuration delay:0 usingSpringWithDamping:10 initialSpringVelocity:10 options:UIViewAnimationOptionCurveEaseInOut animations:^{
        ...
        [self.livingItems enumerateObjectsUsingBlock:^(YQSweepCardItem * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            NSString *count = [NSString stringWithFormat:@"%lu",(unsigned long)idx];
            [numArray addObject:count];
        ...
        NSInteger indexNum =self.itemCount-(unsigned long)numArray.count;
        if (self.index) {
            self.index(indexNum);//控制左滑下标
        }
...
//
//                    if([self.dataSource respondsToSelector:@selector(sweepCardView:itemForIndex:)]){//往右滑
